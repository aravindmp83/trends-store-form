...
// Initialize Supabase (We will add the actual keys securely in Vercel later)
...
export default function StoreForm() {
  ...
  const fetchStore = async () => {
    ...
    if (error || !data) {
      ...
      // Fetch existing postcard data if it's a Chennai store
  ...
  const handleSubmit = async (e) => {
    ...
    // Update Stores Table (City, SM, SM_Mobile)
    ...
    // Update Postcard Table ONLY if City is Chennai
    let pcErr = null;
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-2">
            <input 
              ...
              placeholder="e.g., TR1024"
              value={searchCode}
              onChange={(e) => setSearchCode(e.target.value)}
            />
            <button onClick={fetchStore} className="bg-blue-600 text-white px-4 py-2 rounded font-bold hover:bg-blue-700">
              Fetch
            </button>
          </div>
